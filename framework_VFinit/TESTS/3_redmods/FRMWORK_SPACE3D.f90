...
type(tensor), intent(in) :: t2
! outer(cross or vector) product vector1 .o. vector2
!det=    AxBx     *AyBy     *AzBz  -     AxBx     *AyBz     *AzBy  -     AxBy     *AyBx     *AzBz  +     AxBy  *   AyBz  *   AzBx   
!det= (t2%A(1,1)*t2%A(2,2)*t2%A(3,3) - t2%A(1,1)*t2%A(2,3)*t2%A(3,2) - t2%A(1,2)*t2%A(2,1)*t2%A(3,3) + t2%A(1,2)*t2%A(2,3)*t2%A(3,1) &
!  +   AxBz  *   AyBx  *  AzBy  -    AxBz  *  AyBy  *  AzBx
!   + t2%A(1,3)*t2%A(2,1)*t2%A(3,2) - t2%A(1,3)*t2%A(2,2)*t2%A(3,1)
!(/  AyBy*AzBz - AyBz*AzBy, AxBz*AzBy - AxBy*AzBz, AxBy*AyBz - AxBz*AyBy /)
!(/  AyBz*AzBx - AyBx*AzBz, AxBx*AzBz - AxBz*AzBx, AxBz*AyBx - AxBx*AyBz /)
!(/  AyBx*AzBy - AyBy*AzBx, AxBy*AzBx - AxBx*AzBy, AxBx*AyBy - AxBy*AyBx /)
...
t1%A=t1%A/(t2%A(1,1)*t2%A(2,2)*t2%A(3,3) - t2%A(1,1)*t2%A(2,3)*t2%A(3,2) - t2%A(1,2)*t2%A(2,1)*t2%A(3,3) + t2%A(1,2)*t2%A(2,3)*t2%A(3,1) &
   + t2%A(1,3)*t2%A(2,1)*t2%A(3,2) - t2%A(1,3)*t2%A(2,2)*t2%A(3,1))
end function inv
