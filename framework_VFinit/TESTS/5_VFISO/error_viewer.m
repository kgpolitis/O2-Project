...
filename='~/O2-project/framework_VFinit/TESTS/5_VFISO/area_vol_sph_cart.txt'
...
if (order_line_k~=0) 
    labely(m+1)={['$$ O(x^',int2str(order_line_k),') $$']}
    %labely(m+1)={['$$ O(',xname,'^',int2str(order_line_k),') $$']}
    % add order line
    [max_dv,i_max_dv]=max(dv);
    up=3;
    right=1.5;
    yl=max_dv(1)/(iv(i_max_dv(1)).^order_line_k)*iv.^order_line_k;
    loglog(iv/right,yl*up,'b--','linewidth',2)
end
...
if (order_line_k~=0 && use_fits==1)
    legend(labely,'2nd Order')
elseif (order_line_k~=0)
