...
! Defines the framework for Marching Polyhedra Algorithm
...
private
public :: add, subtract, finalize_Ci
...
type, abstract, public :: fluid_interface
  ...
  type(mf_node),dimension(:), allocatable :: mfnodes
  type(mf_face),dimension(:), allocatable :: mffaces
  type(mf_cell),dimension(:), allocatable :: mfcells
 contains                                                              !
...
type, public :: mf_face
  ...
  logical                                      :: in, out, intersected, iso=.false., ambiguious=.false.
 contains
end type mf_face

type, public :: mf_FV
  real(kind(0.d0))                                      :: Ci
  logical                                               :: in, out, intersected, iso=.false., trimmed=.false.
...
real(kind(0.d0)) elemental function eval_node(sh,node) result(f)
 class(fluid_interface), intent(in) :: sh
 type(abstract_node), intent(in) :: node
...
type(mf_node) elemental function node_in_out_at(sh,no) result(mfno)
!
...
!
 ...
 else if (value_node > sh%almost_at) then ! g(p) > 0
...
type(mf_face) elemental function face_section(sh,fa) result(gfa)
class(fluid_interface), intent(in) :: sh
type(face), intent(in) :: gfa
...
type(point), dimension(:), allocatable :: help_points
 ...
 allocate(in(size(fa%n_nb)),source=sh%mfnodes(gfa%nb%gl_no)%in)
 allocate(out(size(fa%n_nb)),source=sh%mfnodes(gfa%nb%gl_no)%out)
 allocate(at(size(fa%n_nb)),source=sh%mfnodes(gfa%nb%gl_no)%at)
 ...
 if ( j1 == size(fa%n_nb) ) then ! all node are at := face coincides with the interface
    ...
    gfa%at  = .false.
    gfa%iso = .true.
    gfa%Ci  = sh%eval(fa) ! ambiguous Ci ... here the Ci can be both 0 and 1 
    
    ! keep points in point array, one isoedge is defined for each at-at edge
    allocate(gfa%atatedge(size(fa%n_nb)))
    if (gfa%Ci <=0) then
      ...
      do i1=1,size(fa%n_nb)
        ...
        fa%atatedge(i1)%pnt(2) = fa%n_nb(i1)%node%pn
        fa%atatedge(i1)%pnt(1) = fa%n_nb(i1_plus1)%node%pn
        
        ! store the local node integers
        fa%atatedge(i1)%gl_no(2) = i1       
        fa%atatedge(i1)%gl_no(1) = i1_plus1
 ...
 !else if ( count(at) + count(in) == size(fa%n_nb) ) then   ! (A), (C)
    ...
    if ( control_2D ) then                          ! 2D problem
      ...
      morethan2_intps: if ( j+j1 > 2 ) then  ! more than two intersection points
        ...
        ! 
        !   > j2 = number of intermediate intersection points, defined by refinement fictitious edges:
        ...
        ! are (j+j2)/2+"at-at edges". Thus we should conduct (j+j2)/2 searches to find the iso-edges,
        ...
        j = (j+j1)/2 ! totat edges without at-at edges
