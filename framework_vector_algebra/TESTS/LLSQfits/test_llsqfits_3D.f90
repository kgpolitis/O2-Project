...
! find gradient
grad=fit%gradient(O+vector(0.5,0.5,0d0))

print *, ' Gradient(exact)='
print *, gfield(O+vector(0.5,0.5,0d0))
...
print *, ' '
 ...
 type(point), intent(in) :: p
 res = 3d0 + p%x**2 + p%x*p%y + p%x*p%y*p%z + p%x**3 + p%y**3 + p%z**3
 end function cfield
 ...
 type(point), intent(in) :: p
 res =  3d0*p%x**2*ii + 3d0*p%y**2*jj + 3d0*p%z**2*kk + 2d0*p%x*ii +p%y*ii + p%x*jj + p%x*p%z*jj + p%x*p%y*kk + p%y*p%z*ii
 end function cgfield
 
end program test_llsqfits2