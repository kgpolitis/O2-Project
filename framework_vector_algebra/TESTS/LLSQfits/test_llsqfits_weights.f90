...
real(kind(0.d0)), dimension(:), allocatable, target :: fsample
integer :: np, nx, ny
real(kind(0d0)) :: lx, ly, down, up
! gradient
type(vector) :: grad
! for do loops
integer :: i, j 
! pointer for easy swap of fields
...
forall(i=1:nx,j=1:ny) psample((j-1)*nx+i)=O+((i-1)*lx/(nx-1)-lx/2d0)*ii+((j-1)*ly/(ny-1)-ly/2d0)*jj
