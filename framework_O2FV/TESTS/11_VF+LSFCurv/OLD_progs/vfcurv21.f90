...
type(point) :: p0
integer :: i, nx, ny, nz, j,k, k1, i1, nunit, j1, pnt_cnt0, pnt_cnt1=0, pnt_cnt2=0, patch_cnt0=0, patch_cnt1=0, patch_cnt2=0, npasses, ipass, reps, irep, jrep
...
logical :: sec_pass, trd_pass, ex, add_grad, add_correction_grad, interp_pass, at_faces, use_plic, output, tec
...
integer :: RE_vols=400, RE_vols_ex=500, RE_Ci=600, RE_Dist=700, CurvE_ex=800, CurvE_ap=900, grad_type, sub_cell, nrep
...
open(CurvE_ap,file="RE_CurvE.txt",position="append",recl=1000,action="write")
...
open(CurvE_ex,file="RE_CurvE_ex.txt",position="append",recl=1000,action="write")
...
open(RE_vols_ex,file="RE_Vol_ex.txt",position="append",recl=1000,action="write")
...
open(RE_Dist,file="RE_Dist.txt",position="append",recl=1000,action="write")
...
open(RE_Ci,file="RE_Ci.txt",position="append",recl=1000,action="write")
...
open(RE_vols,file="RE_Vol.txt",position="append",recl=1000,action="write")
...
pln%p0 = point(0d0,0d0,l_grid/2d0)

                 ! test 1 
do jrep=1,reps !24
do irep=1,reps

nrep=(jrep-1)*reps+irep

!if (nrep /=1 .and. nrep <1013) cycle  
print *, " REP is ", nrep," n="

!pln%unit_normal = unit(vector(0d0,-1d0+l_grid/2d0,hs(irep)).x.vector(1d0-l_grid/2d0,0d0,hs(jrep)))
pln%unit_normal = unit(vector(0d0,-1d0,hs(irep)).x.vector(1d0,0d0,hs(jrep)))
...
do i=1,size(mffvs)
  ...
  if (allocated(FVs(i)%pnb)) deallocate(FVs(i)%pnb)
...
if (nrep/=1) call my_neigh_opts%init_again
...
if (output) write(RE_Dist,*), nx, pnt_cnt0, patch_cnt0,max_d0,L1_d0, tend-tstart
...
if (output) write(RE_Ci,*), nx, count(used2), maxval(Ci1,used), sum(Ci1,used)/patch_cnt0, maxval(Ci1,used2), sum(Ci1,used2)/count(used2)
...
Aapprox=sum(area)
Aerr=Aexact-Aapprox
if (output) write(RE_vols,*), nx, Aapprox, Vapprox, Aerr, Aerr/Aexact, Verr, Verr/Vexact
deallocate(sgrid)
...
if (output) write(CurvE_ap,*), nx, 2d0/nx, patch_cnt0, count(used), EC_max, EC_L1, EC_L2, max_d0, max_d1, maxval(Ci2)
...
call tecplot(1)%set("out")

call tecplot(1)%set(nodes,faces,fvs,mpi_boundary)

call tecplot(1)%plot(interpfield) !1

call tecplot(1)%plot(field)!2

call tecplot(1)%plot(FVs%Ci)!3

call tecplot(1)%plot(curv_ex)!4

call tecplot(1)%plot(curv)!5
