...
type(point) :: p0
integer :: i, nx, ny, nz, j,k, k1, i1, nunit, j1, pnt_cnt0, pnt_cnt1=0, pnt_cnt2=0, patch_cnt0=0, patch_cnt1=0, patch_cnt2=0
...
logical :: sec_pass, trd_pass, ex, add_grad, add_correction_grad, interp_pass
...
if (parallel_execution) then
  call fv_write_plic_mpi('ex_iso',patches=.true.)
else
  call fv_write_plic_serial('ex_iso',patches=.true.)
end if
...
add_grad = .false. ; add_correction_grad = .true. ; interp_pass = .false.
...
do i1=1,size(faces)
 if (faces(i1)%ivar/=0) then
   field(faces(i1)%ivar)=1d0-Cif(i1)
   !field(faces(i1)%nb(1)%gl_no)=1d0-FVs(faces(i1)%nb(1)%gl_no)%Ci
 end if
end do
...
do i=1,size(FVs)
  ...
  call FVs(i)%capture(interpfield,50d-2,storeCi=.true.,update_sgrid=.true.)
  !call FVs(i)%capture(interpfield,50d-2,storeCi=.true.,bnd_iso=.true.)
...
pnt_cnt2=size(sgrid)
allocate(d2plane,source=abs((sgrid%pn-pln%p0)*pln%unit_normal))
max_d2=maxval(d2plane)
L1_d2=sum(d2plane)/size(d2plane)
deallocate(d2plane)
...
do i1=1,size(FVs)
  if (allocated(FVs(i1)%poiarr)) then 
    patch_cnt2=patch_cnt2+1
    used(i1)=.true.
  end if
end do

write(RE_Dist,*), "     -     ", pnt_cnt2, patch_cnt2,max_d2,L1_d2
...
deallocate(Ci1)
allocate(Ci1,source=Ci_exact-1d0+FVs%Ci)
write(RE_Ci,*), "     -     ", count(used2), maxval(Ci1,used), sum(Ci1,used)/patch_cnt2, maxval(Ci1,used2), sum(Ci1,used2)/count(used2) 
...
if (parallel_execution) then
  ...
  call fv_write_plic_serial('interp_iso',patches=.false.,color=col,bnd=.false.)
end if
