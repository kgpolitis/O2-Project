...
logical :: mpi_init=.false., i_tec = .true., i_maxd_vs_curv=.false.
...
do 
 ...
 if (abs(exact_area-kexact)/Vexact<1d-6) exit
end do
 
!exact_curv = 2d0/wav%radius
!exact_vol  = exact_area*wav%radius/3

call wav%init_VF(isof_remove=.true.,srd=srd,dbg=.true.)
...
call isosurfaces_gen(field=wav%Ci,iso_value=5d-1,sgridgen=sgridgen,storeCi=.true.,gfield=gradCi,gfield4ivars=.true.)
...
call cpu_time(tstart)
call lsfic(curv_iso_sgrid,dist2planeloc=d2pl,dist2patch=d2patch,prob_found=prob)
call cpu_time(tend)
...
call stecplot(2)%plot(prob,"prob_loc")
