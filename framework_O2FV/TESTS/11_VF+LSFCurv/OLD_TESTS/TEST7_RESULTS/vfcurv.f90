...
add_grad = .false. ; add_correction_grad = .false. ; interp_pass = .false. ; at_faces=.false.; use_plic=.true.
...
do i1=1,size(FVs)
  call FVs(i1)%plic_cif(grad(i1),field,-10)
  !call FVs(i1)%plic_cif(pln%unit_normal,field)
end do
...
!do i1=1,size(faces)
!  if (faces(i1)%ivar/=0) field(faces(i1)%nb(1)%gl_no)=field(faces(i1)%ivar)
!end do
