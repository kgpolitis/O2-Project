...
! .........OOOOOOOOOOO.....O&  Note that this code can be only executed serially, for GMSH grids there is  
! ......OOOO........OOOO...N&  no parallel execution for now
...
! read a GMSH grid
call counts_O2_volfile('/home/g/Programming_PhD/Gmsh2Isis/cube_313_opt.msh.vol',nx,ny,nz)
...
if (parallel_execution) then
 ...
 call paraopen(nunit,stem="times_GMSH_para",suffix='txt',position="append",recl=1000)
else
 open(newunit=nunit,file="times_GMSH.txt",position="append",recl=1000)
end if

write(nunit,*), nz, tgrid, tneighs, tcalc
close(nunit)

allocate(neighs_show(size(fvs)),source=0d0)
! element with 160 n1 neighs
!i1=314331
! in boundary
i1=165087
!i1=276540
...
neighs_show(FVs(i1)%neighs(FVs(i1)%neighsj(1)+1:FVs(i1)%neighsj(2))) = 3d0
neighs_show(FVs(i1)%neighs(FVs(i1)%neighsj(2)+1:)) = 4d0
...
!call FVs(i1)%write_neighborhood
