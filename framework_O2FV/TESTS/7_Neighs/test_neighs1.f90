...
! ......OOOO........OOOO...N
! ....OOOO...........OOOO..A  In this case the neighborhoods are generated in specific cells that are 
! ...OOO..............OOO..T  defined by the true values of a tag array. Note that the tag arrays must
! ..OOO................OOO.E  be allocatable arrays. Note that allocatable arrays are always placing data
! .OOO.................OOO.=  in a concecuitive manner which is a good optimization characteristic!!!     
! .OOO................OOO..=
! .OOO...............@@@...L
! .OOOO.............@...@..O
! ..OOOO..........OOO...@..V
...
implicit none
type(point) :: ps,pe
integer :: nx, ny, nz, i1, nunit, j1, tag_count, nn
type(point) :: p0
type(vector) :: n
class(neighs_opts), allocatable :: my_neighs
real(kind(0.d0)), dimension(:), allocatable :: elem_counts1, elem_counts2, totVs, Vc, distmaxppc, elem_tags2,elem_tags3
logical, dimension(:), allocatable :: tags, DGM_tags
...
call cpu_time(tstart)
call findneighs(my_neighs,tags=tags,dbg=.false.,prf=.true.)
call cpu_time(tend)
...
! 
! In this case the neighborhoods are not available everywhere, therefore we should check
! where they are available to get the required info. There are two ways ( -1- and -2- below ) of 
! doing this, either by the using the allocated statement or by using the packing the cells that
! include neighborhoods. Depending on what we want to accomplish one might be better than the other
! 
call cpu_time(tstart)
allocate(elem_counts1(size(FVs)),source=0d0)
allocate(elem_counts2(size(FVs)),source=0d0)
allocate(elem_tags2(size(FVs)),source=0d0)
allocate(elem_tags3(size(FVs)),source=0d0)
...
! tag count must be equal to size(cells)
print *, tag_count, "=?", size(cells),'->',tag_count==size(cells)
...
!     ! level 1 element tags -> to construct level 2
!     elem_tags2(FVs(i1)%neighs(1:FVs(i1)%neighsj(1)))=1
!     
!     ! level 2 elements tags -> to construct level 3
!     elem_tags3(FVs(i1)%neighs(FVs(i1)%neighsj(1)+1:FVs(i1)%neighsj(2)))=1
    
end do way2
! 
! elem_tags3(pack((/1:size(FVs)/),elem_tags2/=0))=0
! elem_tags2(cells)=0
...
if (parallel_execution) then
    ...
    deallocate(Vc)
        ! DGM visualizations > reset elem_counts1
    ! 
    ! Visualize DGM in process 0
    allocate(DGM_tags(mpi_db%ivar_max),source=.false.)
    if (my_rank==0) DGM_tags=.true.
    call mpi_db%inform(DGM_tags)
    elem_counts1=0d0
    where(DGM_tags(1:size(FVs))) elem_counts1=1d0
    
    ! Visualize DGM in process 1
    DGM_tags=.false.
    if (my_rank==1) DGM_tags=.true.
    call mpi_db%inform(DGM_tags)
    elem_counts2=0d0
    where(DGM_tags(1:size(FVs))) elem_counts2=1d0
...
! call tecplot(1)%plot(elem_tags2)
! 
! call tecplot(1)%plot(elem_tags3)
