...
! .OOOO.............@...@..O& but afterwards the levels are extended to be everywhere 3. This is an application
! ..OOOO..........OOO...@..V& of the extend mode of the neighborhood generation
! ....OOOOO...OOOOO....@...I& 
! .......OOOOOO......@@....N& 
! ..................@@@@@@.G  
program test_neighs5
...
! after finding the neighborhoods we use them to perform a calculation... 
...
way2: do j1=1,size(cells)
    ...
    elem_counts1(i1) = size(FVs(i1)%neighs)
    
end do way2

! now we want to extend the neighborhood to perform another calculation(here the same calculation)...
my_neighs%mode = extend
! note that lvl_max is the maximun level we are constructing and not the number of levels we ask!!!
my_neighs%lvl_max = 3

! note that if we didn't have setted mode to extend the all the previous calculation would be 
! repeated, since the mode would be to the default construct mode. When changing the mode care must
! be exercised since the change is sticky and it will be effective for each subsequent call of the
! findneighs subroutine. If this is not desired then you must set the neighs options you use to 
! the default mode which is specified by the keyword construct.
! 
! Note: Probably here we should have anothere subroutine that overloades the name findneighs...
!       I think it would be more elegant...

call findneighs(my_neighs,n1_tags=n1_tags,debug=.true.)
! note that at the call we have skipped the tags mask. The tags play the same role as before, i.e.
! they mask the cells that the extends are conducted. However, the extend mode is only meaningful if
! in a cell the neighborhoods have been already initialized, i.e. if the neighs array in a specific
! cell has not been initialized the subroutine will not perform an extend there. So in this 
! case, adding the tags would have no effect of the subroutine. The n1_tags were normally added
! if we don't use them the subroutine will conduct a dynamic search of the n1 neighs. Generating them
