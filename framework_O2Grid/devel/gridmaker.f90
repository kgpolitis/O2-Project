...
!            z(+)\             \ |                 \ |    -- x_points, y_points, z_points different numbers including 0 and 1.  
!                 V             \|..................\|    -- It may be different for each axis and each uses his own divide function.
!                  V            Z1                        -- LIST OF DIVIDE FUNCTIONS : " linear " , " logarithmic " 
!                                                        ----------------------------------------------------------------
...
end subroutine set_leng              ! ----- SUMMARY END -----  calculates the total length of a given curv |        call curve%set_length                       
...
!    t0 _/                                          t0_/
!   
! where r is a point function representing the curve with the first parameterization  " p=r(ts) "  and (using the procedure 
! the program follows) this is given when the abstract type undefined_curve is extended and the function pos is defined (overidden).
! See for example the construction of type str8. Two solutions are known from the beginning, namely : 
! 
!         "  Solution 1 --> for t=0 we have ts_unknown = t0  "  and  "  Solution 2 --> for t=1 we have ts_unknown = t1 "
! 
! The equation is solved using the bisection method, other methods have been tried but the main problem posed was trial solutions 
! leaving the interval  [t0,t1]. Finally we evaluate the function pos at t_unknown that we have just found --> curve%pos(t_unknown). 
! 
! The function pos_L01 defines:
!      -1- the same curve but using a different parameterization, namely, the normalized arc-length parameterization
!      -2- therefore a new point function. If we name the new function pos_arc then one must have " pos_arc(t)=pos(ts) ".
!      
...
type(point) elemental function pos_L01(lll,t) result(pl)
 ...
 if (t == 0d0) then                                   
    ...
    pl=lll%pos(tr2)   
                      ! -----   SUMMARY   ----- The implimented function pos_L01 calculates |CODE: type(point) :: a     
