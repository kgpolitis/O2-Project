...
real(kind(0.d0)), dimension(:), allocatable, target :: kn, stats
...
FVs%Ci=mffvs%Ci
!FVs%Ci=norm2(FVs%pc-O)-5d-1
!call mpi_boundary%update
deallocate(mffaces,mfnodes,mffvs)

! -> calculate normal and curvature
!call normal_curvature_geom1(.true.,stats)
!call normal_curvature_geom3(.true.)
call normal_curvature_geom9(.true.)
...
!call tecplot(1)%plot(stats)
call tecplot(1)%plot(kn)

call tecplot(1)%update

! ! -> calculate normal and curvature
! call normal_curvature_geom1
! 
