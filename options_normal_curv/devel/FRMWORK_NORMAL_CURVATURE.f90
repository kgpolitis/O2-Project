module frmwork_normal_curvature
 ...
 ! ----  
    ...
    ! --------------------------------
      ...
      integer :: choose_differentiation_method_cu=1
      ! ----    Codes for differentiation method    ----
 ...
 real(kind(0.d0)), dimension(:), allocatable :: divg
 type(vector), dimension(:), allocatable :: grad, ggx, ggy, ggz
 ...
 if ( allocated(i_normal) ) then
    ...
    if ( size(i_normal) /= size(FVs) ) then
     
      deallocate(i_normal,i_curv,field2use)
      allocate(i_normal(size(FVs)),i_curv(size(FVs)),field2use(size(FVs)))
 ...
 case (2)
    ...
    if (ST_like_FT) then
      ...
      if (exact_k) then
        ...
        i_normal = i_curv * safe_unit(grad) * A_plic/FVs%Vc
      end if
